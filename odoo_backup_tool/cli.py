...
def handle_docker_export(args):
    ...
    try:
        ...
        print("\nNext Steps:")
        print(f"  1. Extract the startup script from the archive:")
        print(f"       tar xzf {archive_name} start_odoo_docker.sh")
        print(f"       mv start_odoo_docker.sh ~/scripts/")
        print(f"       chmod +x ~/scripts/start_odoo_docker.sh")
        print(f"  2. Run it:")
        print(f"       ~/scripts/start_odoo_docker.sh")
        print()
        print("  On first run it will extract the archive, build containers,")
        print("  and start Odoo. Subsequent runs auto-detect git-changed")
        print("  modules and upgrade them.")
        print()
        print("  Other modes:")
        print("    --fresh [archive]   Tear down, re-extract, rebuild")
        print("    --all               Upgrade all installed repo modules")
        print("    -u module_name      Upgrade a specific module")
    except Exception as e:
